...
//! `ndtri` (Moshier). The Pearson-correlation lambda search needs the normal
//! quantiles of the uniform order-statistic medians; porting the same rational
//! approximations — the central `|y-0.5| ≤ 3/8` branch and the two
//! `z = sqrt(-2 ln y)` tail branches — keeps those quantiles bit-identical to
//! scipy's.
